...
// Define a class named AdderTest that extends AnyFlatSpec and ChiselScalatestTester
  ...
  "Adder test" should "pass" in {
    ...
    test(new Uint64_Adder) { dut =>   // dut: device under test, refer instance of Uint64_Adder
      ...
      for(testID <- 0 until 10){
        ...
        // Generate random test data within the range of 0 to 2^32 - 1
        val a = BigInt(32, scala.util.Random) // Generate a random 32-bit unsigned integer
        val b = BigInt(32, scala.util.Random) // Generate another random 32
        // 计算生成正确结果(ground truth) 
        val c=a+b
        //  ${} syntax used to embed expressions within string. 
        println(s"a=0x${a.toString(8)} b=0x${b.toString(8)} c=0x${c.toString(8)}")
