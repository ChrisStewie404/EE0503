                        ...
                        )
{
    ...
    val mat_elem_num = matSize*matSize    // A,B,C计算矩阵的大小
    val memory_size = mat_elem_num*3    //memory模块需要足够大，能够容纳A B C三个矩阵，这里假设在memory中，每个矩阵元素都是占用32bit （同学们可以自行优化）
    val matA_baseAddr= 0   // 在内存中存储A矩阵的基地址 (起始地址)
    val matB_baseAddr = mat_elem_num    // 在内存中存储B矩阵的基地址 (起始地址)
    val matC_baseAddr = 2*mat_elem_num  // 在内存中存储C矩阵的基地址 (起始地址)
    val memory_width = resDWidth    //memory中每一个地址对应 memory_width的比特数
}

// TOP 采用了MatMulConfig中的参数化配置
